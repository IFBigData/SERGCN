...
class ResNet_ROI(nn.Module):
    ...
    def forward(self, x, batch_bboxes):
        ...
        batch_size = x.size(0)
        node_num = batch_bboxes.size(1)  # max person
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
...
def resnet101_roi():
    ...
    model_file = '../resnet_model/resnet101-5d3b4d8f.pth'

    logger.info("Loading model {}".format(model_file))

    unload_model_dict = model.state_dict()
    pretrained_dict = torch.load(os.path.join(model_file), map_location=lambda storage, loc: storage)
...
def load_ssl_model():
    ...
    model = ResNet_ROI(Bottleneck, [3, 4, 6, 3])

    model_file = './SSL_model/ssl_model-448.pth'
    # model_file = './SSL_model/ssl_model-448-pipa_pisc.pth'
    pretrained_dict = torch.load(os.path.join(model_file), map_location=lambda storage, loc: storage)
    ...
    unload_model_key = list(unload_model_dict.keys())
    pretrained_key = list(pretrained_dict.keys())[:len(unload_model_dict)]
    tmp = OrderedDict()
    for unkey, prekey in zip(unload_model_key, pretrained_key):
        if unload_model_dict[unkey].shape == pretrained_dict[prekey].shape:
            tmp[unkey] = pretrained_dict[prekey]
    ...
    logger.info("Successfully load model {}".format(model_file))
